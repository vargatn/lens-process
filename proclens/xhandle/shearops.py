...
class StackedProfileContainerALTERNATIVE(object):
    ...
    def _collate(self, ind):
        """concatenates all BUT ind-th patch"""
        pass
...
class StackedProfileContainer(object):
    ...
    def _nullprofiles(self):
        ...
        # Check that we do not have a zero division...
        nomset = set(np.where(np.abs(np.sum(self.data[self.dst_nom, :, :], axis=0)) > 0)[0])
        denomset = set(np.where(np.abs(np.sum(self.data[self.dst_denom, :, :], axis=0)) > 0)[0])
        nzind = sorted(list(nomset.intersection(denomset)))
    ...
    def _subprofiles(self):
        ...
        # FIXME This is now checked elsewhere...
        ...
        # TODO there are somehow zero divisions leaking in from xshear
        ...
        for i, lab in enumerate(self.sub_labels):
            ind = self.indexes[i]
            # cind = hasval[i]
            nomset = set(np.where(np.abs(np.sum(self.data[self.dst_nom, ind, :], axis=0)) > 0)[0])
            denomset = set(np.where(np.abs(np.sum(self.data[self.dst_denom, ind, :], axis=0)) > 0)[0])
            cind = sorted(list(nomset.intersection(denomset)))
    ...
    def prof_maker(self, weights=None):
        ...
        if weights is None:
            ...
            print "be very careful, weights are not fully supported (yet...)"
        self.w = weights
...
class StackedProfileContainerOLD(object):
...
def stacked_pcovOLD(plist):
    ...
    assert isinstance(plist[0], StackedProfileContainerOLD)
